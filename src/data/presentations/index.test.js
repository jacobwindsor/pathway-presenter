...
describe('The presentation data service', () => {
  ...
  it('should have no data when initialized', () => {
    ...
    return presentations
      .get('1234')
      .catch(err =>
        expect(err.message).toEqual('No presentation with ID 1234 found.')
      );
  });
  ...
  it('should only update the wpId', () => {
    return presentations
      .create({ title: 'A title', wpId: 'WP4', slides: [] })
  ...
  it('should list the presentations', () => {
    return presentations
      .create({ title: 'A pres', wpId: 'WP4', version: 0, slides: [] })
      .then(pres =>
        presentations.list().then(theList => {
